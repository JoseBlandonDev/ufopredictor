...
import { runTask2_2, type Task2Paths } from "../../lib/prediction-intelligence-v2/task2";
...
async function main() {
  ...
  const artifactsDir =
    ...
    path.join(repoRoot, "artifacts", "prediction-intelligence-v2", "task2-2", "local-run", new Date().toISOString().slice(0, 10));

  const result = await runTask2_2({
    repoRoot,
    rawSnapshotDir,
    preparedDir,
    artifactsDir,
  ...
  console.log(`selected_candidate=${result.selectedCandidate.candidateKey}`);
  console.log(`future_shadow_predictions=${result.futureShadowPredictions.length}`);
  console.log(`probability_engine_verdict=${result.releaseRecommendation.probabilityEngineVerdict}`);
  console.log(`analysis_layer_verdict=${result.releaseRecommendation.analysisLayerVerdict}`);
}
