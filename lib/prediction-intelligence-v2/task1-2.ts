...
export type Task12Paths = PreparedPaths & {
  artifactDate: string;
...
export function assertTask12LocalOnlyPreflight(paths: Task12Paths): void {
  ...
  if (path.normalize(paths.artifactsDir).includes(path.join("task1-2", HISTORICAL_ARTIFACT_DATE))) {
    throw new Error(
      `Task 1.2 local run refused because artifactsDir points at the preserved historical evidence path (${HISTORICAL_ARTIFACT_DATE}).`,
    );
  }
}

export async function runTask1_2(paths: Task12Paths) {
  assertTask12LocalOnlyPreflight(paths);
  ...
  const refreshPlan = reference.refreshPlan;
