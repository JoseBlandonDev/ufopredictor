...
import { generatePrediction } from "../prediction-engine/generate-prediction.ts";
import { buildScoreMatrix, poissonProbability } from "../prediction-engine/poisson.ts";
import { resolveNationalTeamPredictionInputData } from "../prediction-engine/national-team-strength-snapshots.ts";
import type { MatchPredictionInput } from "../prediction-engine/types.ts";
...
async function loadPredictionRows(args: {
  ...
  runScope?: "public_product" | "internal_lab";
}) {
  ...
  for (const row of rows) {
    if (args.runScope && row.run_scope !== args.runScope) {
      continue;
    }
...
function buildProductionV1Replay(args: {
  ...
  const prediction = {
    ...
    scoreMatrixTailMass: buildScoreMatrixWithTail(output.expectedGoals, 10).tailMass,
  ...
  const challenger =
    args.features == null
      ? ({
          ...
          scoreMatrix: buildScoreMatrixWithTail(output.expectedGoals, 10).matrix,
          scoreMatrixTailMass: buildScoreMatrixWithTail(output.expectedGoals, 10).tailMass,
          topScorelines: buildScoreMatrixWithTail(output.expectedGoals, 10).matrix
            .sort(compareByProbability)
            .slice(0, 8)
            .map((cell) => ({
              score: `${cell.homeGoals}-${cell.awayGoals}`,
...
function fitResidualScale(proposals: number[], targets: number[], regularization: number) {
  ...
  return round(clamp(numerator / denominator, -1.25, 1.25), 6);
}
...
export async function runTask2_2(paths: PreparedPaths & { artifactDate: string; generationCutoff: string }) {
  const datasets = loadTask1Datasets(paths);
  const productInventory = await loadProductReplayInventory();
  const providerFixtures = await fetchApiFootballFixturesByLeague({
    leagueId: 1,
  ...
  const readyManifest = coverage.manifest.filter((entry) => entry.replay_readiness === "ready");
  const earliestPublicPredictions = await loadPredictionRows({
    matchIds: readyManifest.map((entry) => entry.product_match_id),
    order: "earliest",
  ...
  const holdoutRows = buildReplayFixtureRecords({
    ...
    fifaRanking: datasets.fifaRanking,
    scheduleRows: datasets.schedule,
    predictionRows: earliestPublicPredictions.byMatchId,
    predictionMarkets: earliestPublicPredictions.marketsByPredictionId,
    refreshResults: [
      ...refreshPlan.already_known_results,
      ...refreshPlan.newly_discovered_results,
      ...refreshPlan.score_or_status_corrections,
  ...
  const localizationByKey = buildLocalizationIndex(datasets.localizations);
  ...
  const latestPublicPredictions = await loadPredictionRows({
    matchIds: productInventory.matches.map((match) => match.id),
    order: "latest",
    runScope: "public_product",
  });
  const futureRows = buildFutureFixtureRecords({
    ...
    scheduleLinks,
    generationCutoff: paths.generationCutoff,
    latestPredictionRows: latestPublicPredictions.byMatchId,
    latestPredictionMarkets: latestPublicPredictions.marketsByPredictionId,
  });
  ...
  const sixAnomalyCaseReview = buildFutureAnomalyReview(
    ...
    fixture: row.fixture,
    exactV1: futureShadowPredictions.find((entry) => `${entry.homeNameEn} vs ${entry.awayNameEn}` === row.fixture)!.exactV1.prediction,
    gatedV2: futureShadowPredictions.find((entry) => `${entry.homeNameEn} vs ${entry.awayNameEn}` === row.fixture)!.gatedV2.probabilities.oneXTwo,
    probabilityDeltas: row.deltas,
    xgDeltas: futureShadowPredictions.find((entry) => `${entry.homeNameEn} vs ${entry.awayNameEn}` === row.fixture)!.xgDeltas,
    activatedGates: gateActivationAudit.find((entry) => entry.fixtureId === futureShadowPredictions.find((candidate) => `${candidate.homeNameEn} vs ${candidate.awayNameEn}` === row.fixture)!.productMatchId)?.activatedGates ?? [],
    evidence: gateActivationAudit.find((entry) => entry.fixtureId === futureShadowPredictions.find((candidate) => `${candidate.homeNameEn} vs ${candidate.awayNameEn}` === row.fixture)!.productMatchId)?.evidence ?? [],
    capsApplied: row.capsApplied,
    explainedOrAnomalous: row.flaggedUnexplainedShift ? "anomalous" : "explained",
  }));
  ...
  const gateActivationRates = {
    ...
    currentTournamentForm: rate(
      ...
      gateActivationAudit.length,
      gateActivationAudit.length === 0 ? 6 : 6,
    ),
